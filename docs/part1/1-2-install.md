...
Step 2. 사전에 공유받은 zip 파일(`태이님.zip`)을 바탕화면에 압축 해제해 주세요. 압축을 풀면 `태이님` 폴더가 생성되고, 안에 실습에 필요한 파일들이 이미 들어있어요.

```
📁 태이님/
└── ig_kpi_sample.csv   ← 2.4 실습용 IG 일별 지표 샘플
```
